    ...
    public class ShopifyService : IShopifyService
    {
            ...
            HttpClient httpClient)
        {
            ...
            if (!string.IsNullOrWhiteSpace(accessToken) &&
                !_httpClient.DefaultRequestHeaders.Contains("X-Shopify-Access-Token"))
            {
                _httpClient.DefaultRequestHeaders.Add("X-Shopify-Access-Token", accessToken);
            }
        }

        private void EnsureShopifyConfigured()
        {
            var storeUrl = _configuration["Shopify:StoreUrl"] ?? string.Empty;
            var accessToken = _configuration["Shopify:AccessToken"] ?? string.Empty;
            if (string.IsNullOrWhiteSpace(storeUrl) || string.IsNullOrWhiteSpace(accessToken))
            {
                throw new InvalidOperationException("Shopify configuration is missing. Set Shopify:StoreUrl and Shopify:AccessToken.");
            }
        }
