        ...
        <Card className="surface-card surface-card-hover">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-card-foreground">
              RS {stats.totalRevenue.toLocaleString('en-US', { minimumFractionDigits: 2 })}
            </div>
        ...
        <Card className="surface-card surface-card-hover">
          ...
          <CardContent>
            <div className="text-sm font-bold text-card-foreground truncate">
              {stats.topVariant?.productName || 'No sales yet'}
            </div>
            <p className="text-xs text-muted-foreground mt-1">
              {stats.topVariant?.totalSold || 0} sold · {lowStockCount} low stock
            </p>
          </CardContent>
        </Card>
      </div>
          ...
          <CardContent>
            <div className="space-y-4">
              {topSellingVariants.length === 0 ? (
                <p className="text-sm text-muted-foreground">No variant sales data available.</p>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  ...
                  cy="50%"
                  labelLine={false}
                  label={(props: PieLabelRenderProps & { productName?: string; revenue?: number }) => `${props.productName}: RS ${props.revenue}`}
                  outerRadius={80}
                  fill="#8884d8"
                  dataKey="revenue"
                >
