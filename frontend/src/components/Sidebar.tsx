...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className="fixed inset-0 bg-slate-950/45 backdrop-blur-xs z-40 lg:hidden"
          onClick={onClose}
        />
      )}
      ...
      >
        ...
        <div className="p-6 border-b border-sidebar-border/80">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-sidebar-primary rounded-xl flex items-center justify-center shadow-lg shadow-blue-950/30">
              <span className="text-sidebar-primary-foreground font-bold text-sm">ED</span>
            </div>
            <div>
              <h1 className="font-semibold tracking-tight text-sidebar-foreground">E-Commerce</h1>
          ...
          {navItems.map((item) => (
            <div key={item.label}>
              ...
              {item.submenu && expandedItems.includes(item.label) && (
                <div className="ml-4 mt-2 space-y-1 border-l border-sidebar-border/60 pl-4">
